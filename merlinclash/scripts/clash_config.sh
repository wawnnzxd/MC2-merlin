...
restart_dnsmasq() {
	...
	if [ "$link_dns_target" != "/koolshare/merlinclash/conf/dnsmasq.postconf" ]; then
    	ln -sf /koolshare/merlinclash/conf/dnsmasq.postconf /jffs/scripts/dnsmasq.postconf
	fi
	if [ "$link_sdn_target" != "/koolshare/merlinclash/conf/dnsmasq.postconf" ]; then
		ln -sf /koolshare/merlinclash/conf/dnsmasq.postconf /jffs/scripts/dnsmasq-sdn.postconf
	fi
	# Restart dnsmasq
...
creat_ipset() {
	...
	if [ $(ipv6_mode) == "true" ]; then
		...
		fake_ip_range6=$(yq eval '.dns.fake-ip-range6' /koolshare/merlinclash/yaml_dns/fakeip.yaml)
	...
	if [ $(ipv6_mode) == "true" ]; then
		if [ ! -f "/koolshare/res/china_ip_route6.ipset" ]; then
			...
			cp /koolshare/merlinclash/yaml_basic/ChinaIPv6.yaml /tmp/china_ip_route6.list 2>/dev/null
			...
			echo "create china_ip_route6 hash:net family inet6" >/koolshare/res/china_ip_route6.ipset
			awk '!/^$/&&!/^#/{printf("add china_ip_route6 %s'" "'\n",$0)}' /tmp/china_ip_route6.list >>/koolshare/res/china_ip_route6.ipset
	...
	if [ -f "/koolshare/merlinclash/yaml_basic/ipsetproxy.yaml" ]; then
		...
		sh /koolshare/scripts/clash_ipsetproxy.sh 1 ipsetproxy
	fi
	if [ -f "/koolshare/merlinclash/yaml_basic/ipsetproxyarround.yaml" ]; then
		echo_date "开始创建强制绕行规则到ipset规则集..." >> $LOG_FILE
		sh /koolshare/scripts/clash_ipsetproxy.sh 1 ipsetproxyarround
	fi

}

clean_ipset(){
	rm -rf /koolshare/merlinclash/conf/ipsetproxyarround.conf >/dev/null 2>&1
	rm -rf /jffs/configs/dnsmasq.d/ipsetproxyarround.conf >/dev/null 2>&1
	rm -rf /koolshare/merlinclash/conf/ipsetproxy.conf >/dev/null 2>&1
	rm -rf /jffs/configs/dnsmasq.d/ipsetproxy.conf >/dev/null 2>&1
}
...
check_yaml(){
	...
	if [ -f "$yamlpath" ]; then
		...
		# 	#合并rule_bak默认文件
		# 	cat /koolshare/merlinclash/rule_bak/${yamlname}_rules.yaml >> $yamlpath
		# else
		# 	cat /koolshare/merlinclash/rule_use/${yamlname}_rules.yaml >> $yamlpath
		# fi
		#拼接头文件
		sed -i '$a' $yamlpath
		cat /koolshare/merlinclash/yaml_basic/head.yaml >> $yamlpath
		echo_date "标准头文件合并完毕" >> $LOG_FILE

		#拼接Hosts文件
		echo_date "拼接Host文件" >> $LOG_FILE
		sed -i '$a' $yamlpath
		cat /koolshare/merlinclash/yaml_basic/hosts.yaml >> $yamlpath
		#SNIFFER拼合
...
fixTimeZone() {
    ...
    [ ! -e "/etc/localtime" ] && [ -f "/koolshare/merlinclash/Shanghai" ] &&
    ln -sf /koolshare/merlinclash/Shanghai /etc/localtime
}
...
startClashNormalOrPerp(){
    ...
    if [ "$watchdog" = "1" ];then
		...
		mkdir -p /koolshare/perp/clash
		cat >/koolshare/perp/clash/rc.main <<-EOF
			#!/bin/sh
			source /koolshare/scripts/base.sh
			CMD="/koolshare/bin/clash -d /koolshare/merlinclash/ -f $yamlpath 1>$clashRunLog  2>&1 &"
			if test \${1} = 'start' ; then
				exec >$clashRunLog 2>&1
				exec \$CMD
			fi
			exit 0

		EOF
		chmod +x /koolshare/perp/clash/rc.main
		chmod +t /koolshare/perp/clash/
		sync
		perpctl A clash >/dev/null 2>&1
		perpctl u clash >/dev/null 2>&1
    else
        /koolshare/bin/clash -d /koolshare/merlinclash/ -f $yamlpath 1>$clashRunLog  2>&1 &
    fi
}
...
lan_bypass(){	
	...
	if [ "$mnm" != "3" ]; then
		echo "create lan_ip_blacklist hash:net family inet hashsize 1024 maxelem 65536" >/koolshare/res/lan_ip_blacklist.ipset
		echo "create lan_ip_whitelist hash:net family inet hashsize 1024 maxelem 65536" >/koolshare/res/lan_ip_whitelist.ipset
	fi
	if [ "$mnm" != "2" ]; then
		echo "create lan_mac_blacklist hash:mac hashsize 1024 maxelem 65536" >/koolshare/res/lan_mac_blacklist.ipset
		echo "create lan_mac_whitelist hash:mac hashsize 1024 maxelem 65536" >/koolshare/res/lan_mac_whitelist.ipset	
	fi
	if [ "$list_flag" == "1" ]; then
		if [ -n "$nokpacl_nu" ]; then
			for nokpacl in $nokpacl_nu; do
				...
				if [ "$proxy_mode" == "0" ]; then
					...
					[ -n "$macaddr" ] && echo "add lan_mac_blacklist ${macaddr}" >> /koolshare/res/lan_mac_blacklist.ipset
					[ -n "$macaddr" ] && echo "add macblacklist_dns ${macaddr}" >> /koolshare/res/macblacklist_dns.ipset
					[ -n "$ipaddr" ] && echo "add lan_ip_blacklist ${ipaddr}" >> /koolshare/res/lan_ip_blacklist.ipset
					[ -n "$ipaddr" ] && echo "add ipblacklist_dns ${ipaddr}" >> /koolshare/res/ipblacklist_dns.ipset
				fi
				if [ "$proxy_mode" == "1" ] && [ "$ports" == "all" ]; then
					# echo_date "$proxy_name 全端口转发进Clash，添加进白名单集和转发DNS集" >> $LOG_FILE
					[ -n "$macaddr" ] && echo "add lan_mac_whitelist ${macaddr}" >> /koolshare/res/lan_mac_whitelist.ipset
					[ -n "$macaddr" ] && echo "add macwhitelist_dns ${macaddr}" >> /koolshare/res/macwhitelist_dns.ipset
					[ -n "$ipaddr" ] && echo "add lan_ip_whitelist ${ipaddr}" >> /koolshare/res/lan_ip_whitelist.ipset
					[ -n "$ipaddr" ] && echo "add ipwhitelist_dns ${ipaddr}" >> /koolshare/res/ipwhitelist_dns.ipset
				fi
				if [ "$proxy_mode" == "1" ] && [ "$ports" != "all" ]; then
			...
			if [ "$mnm" != "2" ]; then
				...
				ipset -! restore </koolshare/res/lan_mac_blacklist.ipset 2>/dev/null
				...
				ipset -! restore </koolshare/res/lan_mac_whitelist.ipset 2>/dev/null
			...
			if [ "$mnm" != "3" ]; then
				...
				ipset -! restore </koolshare/res/lan_ip_blacklist.ipset 2>/dev/null
				...
				ipset -! restore </koolshare/res/lan_ip_whitelist.ipset 2>/dev/null
...
write_update_yaml_cron(){
    ...
    yaml_dlinks_file=/koolshare/merlinclash/yaml_bak/${yamlname}.dlinks
		...
		if [ "$mcenable" == "1" ] && [ "${clash_process_started}" = "1" ]; then
        	...
        	case "${update_time}" in
				86400)
					...
					cru a autoupdate "0 5 * * * /bin/sh /koolshare/scripts/clash_subscribe.sh cron cron"
					cru a autologdel "0 * * * * /bin/sh /koolshare/scripts/clash_logautodel.sh"
				;;
				259200)
					echo_date "设置每周一周四凌晨5点，更新订阅配置" >> $LOG_FILE
					cru a autoupdate "0 5 * * 1,4 /bin/sh /koolshare/scripts/clash_subscribe.sh cron cron"
					cru a autologdel "0 * * * * /bin/sh /koolshare/scripts/clash_logautodel.sh"
				;;
				604800)
					echo_date "设置每周一凌晨5点，更新订阅配置" >> $LOG_FILE
					cru a autoupdate "0 5 * * 1 /bin/sh /koolshare/scripts/clash_subscribe.sh cron cron"
					cru a autologdel "0 * * * * /bin/sh /koolshare/scripts/clash_logautodel.sh"
				;;
				*)
...
start)
	set_lock
	#日志自动删除，防止长时间未重启文件过大
	sh /koolshare/scripts/clash_logautodel.sh
