...
decode_url_link(){
	if [ -f "/koolshare/bin/base64_decode" ]; then
		...
		if [ "$mod4" -gt "0" ]; then
			...
			echo -n "$newlink" | sed 's/-/+/g; s/_/\//g' | base64_decode -d 2>/dev/null
		else
			echo -n "$link" | sed 's/-/+/g; s/_/\//g' | base64_decode -d 2>/dev/null
		fi
	else
		echo_date "【错误】固件缺少base64decode文件，无法正常订阅，直接退出" >> $LOG_FILE
		echo_date "解决办法请查看MerlinClash Wiki" >> $LOG_FILE
...
encode_url_link(){
	if [ -f "/koolshare/bin/base64_encode" ]; then
		local link=$1
		# 使用 base64_encode 编码，然后转换为 URL 安全格式
		# 将 + 替换为 -, / 替换为 _, 并移除末尾的 = 填充
		echo -n "$link" | base64_encode 2>/dev/null | sed 's/+/-/g; s/\//_/g; s/=*$//'
	else
