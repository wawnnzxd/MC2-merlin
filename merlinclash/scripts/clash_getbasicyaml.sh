...
ln -sf /koolshare/merlinclash/yaml_basic/head.yaml /tmp/upload/clash_head.txt
#dns
...
if [ -f "/koolshare/merlinclash/rule_custom/${merlinclash_set_yamlsel_start}_custom_rule.yaml" ]; then
	ln -sf /koolshare/merlinclash/rule_custom/${merlinclash_set_yamlsel_start}_custom_rule.yaml /tmp/upload/clash_rule.txt
fi
